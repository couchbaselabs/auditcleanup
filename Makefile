...
LDFLAGS = "-s -w -X github.com/couchbase/auditcleanup/pkg/version.version=$(version) -X github.com/couchbase/auditcleanup/pkg/version.revision=$(revision) -X github.com/couchbase/auditcleanup/pkg/version.buildNumber=$(bldNum) -X github.com/couchbase/auditcleanup/pkg/version.gitRevision=$(GIT_REVISION)"

# Hardcode version values for testing
TEST_LDFLAGS = "-X github.com/couchbase/auditcleanup/pkg/version.version=1 -X github.com/couchbase/auditcleanup/pkg/version.revision=2 -X github.com/couchbase/auditcleanup/pkg/version.buildNumber=3 -X github.com/couchbase/auditcleanup/pkg/version.gitRevision=456"
...
build: $(SOURCE) go.mod
	for platform in linux darwin ; do \
	  ...
	  GOOS=$$platform GOARCH=amd64 CGO_ENABLED=0 GO111MODULE=on go build -trimpath -ldflags $(LDFLAGS) -o bin/$$platform/couchbase-audit-cleanup ./cmd ; \
	done

image-artifacts: build
	mkdir -p $(ARTIFACTS)/bin/linux
	cp bin/linux/* $(ARTIFACTS)/bin/linux
...
container: build
	docker build -f Dockerfile --target=production --build-arg PROD_VERSION=$(version) -t ${DOCKER_USER}/audit-cleanup:${DOCKER_TAG} .

container-rhel: build
	docker build -f Dockerfile.rhel --target=rhel-production --build-arg OPERATOR_BUILD=$(OPERATOR_BUILD) --build-arg OS_BUILD=$(BUILD) --build-arg PROD_VERSION=$(version) -t ${DOCKER_USER}/audit-cleanup-rhel:${DOCKER_TAG} .
